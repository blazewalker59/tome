...
/**
 * Wrap a handler so any thrown error is logged with its full cause chain
 * before being rethrown. Drizzle's "Failed query: …" errors keep the real
 * postgres error on `.cause`, which TanStack Start's serializer drops on
 * the way to the client — logging here is the only way to see the actual
 * SQLSTATE / message in Worker tail.
 */
...
/**
 * Create a new editorial pack. The pack kind is hard-coded to `editorial`
 * here — `deck` packs are only ever generated by the (future) deck-pack
 * flow, which owns `sourceDeckId` and has its own creation path.
 *
 * Slug is normalized to kebab-case and guaranteed unique by a DB
 * constraint; we surface a friendlier error than the raw constraint
 * violation by checking up-front.
 */
  ...
  .handler(
    withErrorLogging("createPackFn", async ({ data }): Promise<CreatePackResult> => {
      ...
      if (existing.length > 0) {
        throw new Error(`Pack slug "${slug}" already exists`);
      }
  ...
  .handler(
    withErrorLogging("getPackFn", async ({ data }): Promise<AdminPackDetail> => {
      ...
      return {
        ...
        kind: pack.kind as "editorial" | "deck",
